...
import {
  ...
  type OpenClawStartupCompatibilityMode,
...
const CLI_ERROR_TYPE = 'cli_error';
...
export function extractOpenClawCliFailure(stdout: string, stderr: string): string | undefined {
  ...
  for (const candidate of [output, ...output.split(/\r?\n/).reverse()]) {
    try {
      ...
      if (value?.ok === false && value.error?.type === CLI_ERROR_TYPE
...
export async function runOpenClawStartupCompatibility(params: {
  ...
  try {
    ...
    return { status: OpenClawStartupMigrationStatus.Failed, error: error instanceof Error ? error.message : String(error) };
  }
}
